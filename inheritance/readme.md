Prototype
Inheritance is passing down characteristics from a parent to a child, so a child can use that piece of code. A child can call inherited method from the parent, or it can override specific method if adjustment is required.
